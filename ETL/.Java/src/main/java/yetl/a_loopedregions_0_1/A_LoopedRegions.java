...
package yetl.a_loopedregions_0_1;
...
/**
 * Job: A_LoopedRegions Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.3.0.20161026_1219
 * @status 
 */
public class A_LoopedRegions implements TalendJob {
	...
	private final String jobName = "A_LoopedRegions";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					A_LoopedRegions.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(A_LoopedRegions.this, new Object[] { e,
	...
	public static class row4Struct implements
		...
		final static byte[] commonByteArrayLock_YETL_A_LoopedRegions = new byte[0];
		static byte[] commonByteArray_YETL_A_LoopedRegions = new byte[0];
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime
						* result
						+ ((this.LoopedRegionId == null) ? 0
								: this.LoopedRegionId.hashCode());
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_YETL_A_LoopedRegions.length) {
					if (length < 1024
							&& commonByteArray_YETL_A_LoopedRegions.length == 0) {
						commonByteArray_YETL_A_LoopedRegions = new byte[1024];
					} else {
						commonByteArray_YETL_A_LoopedRegions = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_YETL_A_LoopedRegions, 0, length);
				strReturn = new String(commonByteArray_YETL_A_LoopedRegions, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_YETL_A_LoopedRegions) {
		...
		public int compareTo(row4Struct other) {
			...
			returnValue = checkNullsAndCompare(this.LoopedRegionId,
					other.LoopedRegionId);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public static class updatesStruct implements
			routines.system.IPersistableRow<updatesStruct> {
		final static byte[] commonByteArrayLock_YETL_A_LoopedRegions = new byte[0];
		static byte[] commonByteArray_YETL_A_LoopedRegions = new byte[0];
		protected static final int DEFAULT_HASHCODE = 1;
		protected static final int PRIME = 31;
	...
	public static class createsStruct implements
			routines.system.IPersistableRow<createsStruct> {
		final static byte[] commonByteArrayLock_YETL_A_LoopedRegions = new byte[0];
		static byte[] commonByteArray_YETL_A_LoopedRegions = new byte[0];
		protected static final int DEFAULT_HASHCODE = 1;
		protected static final int PRIME = 31;
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				String update_tPostgresqlOutput_1 = "UPDATE \""
						...
						+ "\" SET \"Active\" = ? WHERE \"LoopedRegionId\" = ?";
				...
				if (execStat) {
					if (resourceMap.get("inIterateVComp") == null) {
						...
						runStat.updateStatOnConnection("updates" + iterateId,
								0, 0);
				...
				String insert_tPostgresqlOutput_2 = "INSERT INTO \""
						...
						+ "\" (\"Name\",\"Active\",\"LoopedRegionId\") VALUES (?,?,?)";
				...
				if (execStat) {
					if (resourceMap.get("inIterateVComp") == null) {
						...
						runStat.updateStatOnConnection("creates" + iterateId,
								0, 0);
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(
									workbook_tFileInputExcel_1, "Regioes",
									false));
							...
							if (row1 != null) {
								...
								{ // start of Var scope
									...
									if (rejectedInnerJoin_tMap_1) {
										...
										creates_tmp.LoopedRegionId = java.util.UUID
												.randomUUID();
										creates = creates_tmp;
								...
								if (updates != null) {
									...
									pstmt_tPostgresqlOutput_1.setBoolean(1,
											row3.Activo);
									...
									if (row3.LoopedRegionId == null) {
										...
										pstmt_tPostgresqlOutput_1.setObject(
												2 + count_tPostgresqlOutput_1,
												row3.LoopedRegionId);
									}
								...
								if (creates != null) {
									...
									if (row4.Nome == null) {
										...
										pstmt_tPostgresqlOutput_2.setString(1,
												row4.Nome);
									}

									pstmt_tPostgresqlOutput_2.setBoolean(2,
											row4.Activo);
									...
									if (row4.LoopedRegionId == null) {
										...
										pstmt_tPostgresqlOutput_2.setObject(3,
												row4.LoopedRegionId);
									}
	...
	public void tPostgresqlInput_1Process(
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				// inputs:(after_tFileInputExcel_1) outputs:(row2,row2) | target
				// node:tAdvancedHash_row2 - inputs:(row2) outputs:()
				// linked node: tMap_1 - inputs:(row1,row2)
				// outputs:(updates,creates)
					...
					while (rs_tPostgresqlInput_1.next()) {
						...
						if (colQtyInRs_tPostgresqlInput_1 < 1) {
							...
							if (rs_tPostgresqlInput_1.getObject(1) != null) {
								row2.Active = rs_tPostgresqlInput_1
										.getBoolean(1);
							} else {
								throw new RuntimeException(
						...
						if (colQtyInRs_tPostgresqlInput_1 < 2) {
							...
							if (rs_tPostgresqlInput_1.getObject(2) != null) {
								row2.LoopedRegionId = rs_tPostgresqlInput_1
										.getObject(2);
							} else {
								throw new RuntimeException(
										"Null value in non-Nullable column");
	...
	public static void main(String[] args) {
		final A_LoopedRegions A_LoopedRegionsClass = new A_LoopedRegions();

		int exitCode = A_LoopedRegionsClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = A_LoopedRegions.class
					.getClassLoader().getResourceAsStream(
							"yetl/a_loopedregions_0_1/contexts/" + contextStr
									+ ".properties");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : A_LoopedRegions");
		}
...
/************************************************************************************************
 * 124086 characters generated by Talend Open Studio for Data Integration on the
 * 8 de Setembro de 2018 18:57:23 BST
 ************************************************************************************************/
